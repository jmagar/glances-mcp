        ...
        try:
            ...
            for rule, alerts in rule_counts.items():
                if len(alerts) >= 3:  # 3 or more occurrences
                    alert_insights["recurring_issues"].append({
                        "rule_name": rule,
                        "occurrences": len(alerts),
                        "servers_affected": len(set(a.server_alias for a in alerts)),
            ...
            if alert_insights["resolution_patterns"]:
                avg_res_min = alert_insights["resolution_patterns"]["average_resolution_minutes"]
                if avg_res_min > 60:
    ...
    async def capacity_history() -> str:
        ...
        try:
                    ...
                    if baseline and trend:
                        ...
                        if trend["direction"] == "increasing" and trend["confidence"] > 0.7:
                            if baseline.baseline_value > 70 and trend["recent_change"] > 10:
                                server_capacity["capacity_warnings"].append(
            ...
            for server_data in capacity_data.values():
                if "cpu.total" in server_data["current_utilization"]:
                    cpu_values.append(server_data["current_utilization"]["cpu.total"]["baseline_value"])
                if "mem.percent" in server_data["current_utilization"]:
                    memory_values.append(server_data["current_utilization"]["mem.percent"]["baseline_value"])
                
                # Count increasing trends
                for metric_trend in server_data["growth_trends"].values():
                    if metric_trend["direction"] == "increasing":
