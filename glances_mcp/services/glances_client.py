...
from glances_mcp.config.models import GlancesServer, HealthStatus, ServerStatus
...
class GlancesApiError(Exception):
    ...
    def __init__(self, message: str, server_alias: str, status_code: int | None = None):
...
class GlancesClient:
    ...
    async def _make_request(self, endpoint: str, correlation_id: str | None = None) -> dict[str, Any]:
        """Make an HTTP request to Glances API that returns a dict."""
        result = await self._make_raw_request(endpoint, correlation_id)
        if isinstance(result, dict):
            return result
        else:
            return {"data": result}  # Wrap non-dict responses

    async def _make_list_request(self, endpoint: str, correlation_id: str | None = None) -> list[dict[str, Any]]:
        """Make an HTTP request to Glances API that returns a list."""
        result = await self._make_raw_request(endpoint, correlation_id)
        # If the result is wrapped in a dict with a 'data' key, extract it
        if "data" in result:
            data = result["data"]
            if isinstance(data, list):
                return data
        # Handle case where endpoint directly returns list data in wrapped format
        # For list endpoints, Glances may return the list directly or wrapped
        return cast(list[dict[str, Any]], [])

    async def _make_raw_request(self, endpoint: str, correlation_id: str | None = None) -> Any:
        """Make a raw HTTP request to Glances API."""
        ...
        try:
            ...
            if self.session is None:
                raise GlancesApiError("Session not initialized", self.server.alias)
            ...
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    ...
                    # Ensure we return a dict as expected by the type annotation
                    if isinstance(data, dict):
                        return data
                    else:
                        return {"data": data}  # Wrap non-dict responses
            ...
            raise GlancesApiError("Request timeout", self.server.alias) from e
        except aiohttp.ClientConnectionError as e:
            raise GlancesApiError("Connection error", self.server.alias) from e
        except Exception as e:
            if isinstance(e, GlancesApiError):
                raise
            raise GlancesApiError(f"Unexpected error: {str(e)}", self.server.alias) from e
    ...
    async def get_disk_usage(self) -> list[dict[str, Any]]:
        ...
        return await self._make_list_request("fs")

    async def get_disk_io(self) -> list[dict[str, Any]]:
        """Get disk I/O statistics."""
        return await self._make_list_request("diskio")

    async def get_network_interfaces(self) -> list[dict[str, Any]]:
        """Get network interface statistics."""
        return await self._make_list_request("network")

    async def get_network_connections(self) -> list[dict[str, Any]]:
        """Get network connections."""
        try:
            return await self._make_list_request("connections")
    ...
    async def get_processes(self) -> list[dict[str, Any]]:
        """Get process list."""
        return await self._make_list_request("processlist")

    async def get_containers(self) -> list[dict[str, Any]]:
        """Get Docker container information."""
        try:
            return await self._make_list_request("containers")
        except GlancesApiError:
...
class GlancesClientPool:
    ...
    def get_client(self, server_alias: str) -> GlancesClient | None:
