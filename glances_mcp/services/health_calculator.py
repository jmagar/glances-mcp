...
class HealthCalculator:
    ...
    async def calculate_server_health(
        ...
        # Get typed references to the lists for easier access (prefixed with underscore to indicate unused)
        _critical_issues: list[str] = health_data["critical_issues"]
        _warnings: list[str] = health_data["warnings"]
    ...
    def _calculate_network_health_score(self, network_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if overall_error_rate == 0:
            ...
            score = max(0.0, 40.0 - (overall_error_rate * 5.0))
